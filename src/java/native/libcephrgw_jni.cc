			...
			struct stat *st, uint32_t mask, uint32_t flags )
{
	...
	if ( ret < 0 )
		return true; // allow readdir() to continue past damaged object
    ...
    jlong j_rgw_fs, jlong j_fh, jstring j_path, jstring j_name, jint j_mode )
{
	...
	ret = rgw_lookup( rgw_fs, root_fh, c_path, &rgw_fh, nullptr, 0, RGW_LOOKUP_FLAG_DIR );
	if ( ret < 0 )
	{
		handle_error( env, ret );
		return false;
    ...
    jlong j_rgw_fs, jlong j_fd, jlong j_offset, jbyteArray j_buf, jlong j_size )
{
	...
	ldout( cct, 10 ) << "jni: read: fd " << (int) j_fd << " len " << (size_t) j_size <<
	" offset " << (uint64_t) j_offset << dendl;

	ret = rgw_read( rgw_fs, fh, (uint64_t) j_offset, (size_t) j_size, &bytes_read, c_buf, RGW_READ_FLAG_NONE );
    ...
    jlong j_rgw_fs, jlong j_fd, jlong j_offset, jbyteArray j_buf, jlong j_size )
{
	...
	ldout( cct, 10 ) << "jni: write: fd " << (int) j_fd << " len " << (size_t) j_size <<
	" offset " << (uint64_t) j_offset << dendl;

	ret = rgw_write( rgw_fs, fh, (uint64_t) j_offset, (size_t) j_size, &bytes_written, c_buf, RGW_WRITE_FLAG_NONE );
